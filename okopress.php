<?php
    ...
    $urls = array(
        "https://oko.press/feed"
    );

    // FILE WITH ALREADY POSTED ARTICLES (COUNTAIN LINKS)
    $file = file_get_contents("okopress.txt");
        ...
        if(!empty($feeds))
        {
            ...
            foreach ($feeds->channel->item as $item)
            {
                ...
                else
                {
                    ...
                    // REMOVE UNWANTED TAG FROM DESCRIPTION
                    $description = str_replace("<![CDATA[", "", $description);
                    $description = str_replace("]]>", "", $description);
                    $description = str_replace("[&#8230;]", "...", $description);
                    $description = str_replace("&#8230;", "...", $description);
                    $description = str_replace("[&#8222];", "\"", $description);
                    $description = str_replace("&#8222;", "\"", $description);
                    $description = str_replace("[&#8221]", "\"", $description);
                    $description = str_replace("&#8221", "\"", $description);
    ...
    file_put_contents("okopress.txt", $file);
?>
